...
export function buildInitialPrompt(input: BuildInitialPromptInput): string {
  ...
  return [
    ...
    '',
    'To request actions, emit exactly one code block with language/title conduit:',
    '```conduit',
    '{ "type": "conduit.request.v1", "actions": [{ "id": "read_file", "tool": "file.read", "args": { "path": "README.md" }, "reason": "Need context.", "risk": "low" }] }',
    '```',
    '',
